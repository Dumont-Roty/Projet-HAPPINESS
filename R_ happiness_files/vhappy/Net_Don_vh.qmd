...
dataTrain_att <- ID_v2 %>% dplyr::slice(train)
dataTest_att <- ID_v2 %>% dplyr::slice(-train)

rec_att <- recipe(vhappy~., data = ID_v2) %>% 
...
N <- round(2/3*n) # Taille de l'échantillon d'entrainement
train <- sample(1:n, size = N) # découpage de la data train
dataTrain_i <- ID_v3 %>% dplyr::slice(train)
dataTest_i <- ID_v3 %>% dplyr::slice(-train)

# induce some missing data at random
set.seed(1)

rec_i <- recipe(vhappy~.,data = ID_v3) %>% 
...
N <- round(2/3*n) # Taille de l'échantillon d'entrainement
train <- sample(1:n, size = N) # découpage de la data train
dataTrain_tv <- ID_v5 %>% dplyr::slice(train)
dataTest_tv <- ID_v5 %>% dplyr::slice(-train)

rec_tv <- recipe(vhappy~., data = ID_v5) %>% 
...
```{r}
write.csv(ID_v7, "vhappy.csv", row.names = FALSE)  # Enregistre sans les numéros de ligne
```
