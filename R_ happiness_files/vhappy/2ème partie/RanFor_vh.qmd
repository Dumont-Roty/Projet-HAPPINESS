...
#Ici on a définit nvar commme le nombre de variable totale moins la variable à prédire (soit ici 13)
```
...
```{r}
data.frame(nb_trees = 1:rf$ntree,
           random_forest = rf$err.rate[,2:3],
           bagging = bag$err.rate[,2:3]) %>% 
  tidyr::pivot_longer(cols = c(random_forest.no, random_forest.yes, bagging.no, bagging.yes),
...
for (k in prof) {
  rfprobag <- randomForest(vhappy ~ .,
    data = train_data, method = "class", ntree = 200,
...
```{r}
expand_grid(mtry = 1:3, trees = c(100, 50, 200, 500)) # construction d'un tibble
expand.grid(mtry = 1:3, trees = c(100, 50, 200, 500)) # construction d'un datatab
crossing(mtry = 1:3, trees = c(100, 50, 200, 500))#valeurs ordonné
crossing(mtry = 1:3, trees = c(100, 50, 200, 500), min_n = c(2, 5, 10, 20, 50))
```

```{r}
# préciser ou changer les étendues
rf_param <- extract_parameter_set_dials(tune_rf_wf) |> #on extrait les paramètres
  update(mtry = mtry(c(1,13)), trees = trees(c(50,500))) 
