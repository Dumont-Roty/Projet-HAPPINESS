...
```{r}
#library(FactoMineR)
#var.quanti <- ID_v6[,c(4,10,17)]

#res.pca <- PCA(var.quanti, graph = T)
```
...
```{r}
#dat_rec <- dat_train |> recipe(vhappy~educ+attend+tvhours)
...
# 5 couches qui respectent tjrs le nombre gens très heureux
```
...
```{r}
svm_lin_result |> roc_curve(vhappy, .pred_yes) |> autoplot()
roc(data_test$vhappy, svm_lin_result$.pred_yes) %>% auc()
```
